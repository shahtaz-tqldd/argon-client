...
const editorDefinitions = {
  ...
  persona: {
    ...
    fields: [
      ...
      },
      {
        ...
        type: "select",
        options: ["English", "Bengali", "Spanish", "French", "Portuguese"],
      },
    ],
  },
  messages: {
  ...
  response: {
    ...
    fields: [
      ...
      },
      {
        ...
        type: "toggle",
        help: "Coming soon. Keep this disabled until web search is supported.",
      },
    ],
  },
  escalation: {
  ...
  notFound: {
    ...
    fields: [
      ...
      {
        key: "collectContact",
        label: "Ask for contact information before escalating",
        type: "toggle",
      },
    ],
  },
...
const initialConfig = {
  identity: {
    ...
    name: "Atlas Support",
    description:
      "Customer support assistant for product, billing, and account questions.",
    status: "Active",
  },
  persona: {
    instructions:
      "You are Atlas, a thoughtful support specialist. Use the knowledge base as your source of truth and make every response actionable.",
    personality: "Helpful and professional",
    language: "English",
  },
  messages: {
    welcome: "Hi there! 👋 I’m Atlas. How can I help you today?",
    fallback:
      "I’m not completely sure about that. I can connect you with a teammate who can help.",
  },
  response: {
    instructions:
      "Answer clearly, lead with the direct solution, and use short steps only when they improve understanding.",
    tone: "Friendly",
    length: "Balanced",
    webSearch: false,
  },
  escalation: {
    when: "The visitor asks for a refund, reports a payment issue, requests account deletion, or asks twice for a human.",
    neverAnswer:
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(event) => {
                        ...
                        if (file) update(field.key, URL.createObjectURL(file));
                      }}
                    />
                  </label>
                );
  ...
  return (
    <div className="grid items-stretch gap-5 lg:grid-cols-2">
      <SectionCard
        ...
        description="Name, description, avatar, and availability."
        onEdit={() => edit("identity")}
      >
        <div className="flex items-center gap-4 rounded-2xl bg-muted/30 p-4">
          <span className="flex size-14 items-center justify-center overflow-hidden rounded-2xl bg-primary font-bold text-primary-foreground">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <p className="font-bold">{config.identity.name}</p>
      ...
      <SectionCard
        ...
        description="The core identity and communication style of your AI."
        onEdit={() => edit("persona")}
      >
        <p className="line-clamp-3 rounded-2xl border bg-muted/20 p-4 text-xs leading-5 text-muted-foreground">
          “{config.persona.instructions}”
        </p>
        <div className="mt-3 divide-y">
          <ValueRow label="Personality" value={config.persona.personality} />
          <ValueRow label="Default language">
            <span className="flex items-center gap-1.5 text-xs font-semibold">
              <Languages className="size-3.5 text-primary" />
              {config.persona.language}
            </span>
          </ValueRow>
        </div>
      </SectionCard>
      ...
      <SectionCard
        ...
        description="Welcome visitors and handle uncertain answers consistently."
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border bg-muted/20 p-4">
            ...
            <p className="mt-2 text-sm leading-6">{config.messages.welcome}</p>
          </div>
          <div className="rounded-2xl border bg-muted/20 p-4">
            <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
              Fallback response
            </p>
            <p className="mt-2 text-sm leading-6">{config.messages.fallback}</p>
          </div>
        </div>
      </SectionCard>
    </div>
...
function BehaviorTab({ config, edit }) {
  return (
    <div className="grid items-stretch gap-5 lg:grid-cols-2">
      <SectionCard
        icon={BrainCircuit}
        title="AI response behavior"
        description="Instructions, tone, length, and external knowledge controls."
        onEdit={() => edit("response")}
      >
        <div className="divide-y">
          <ValueRow label="Tone" value={config.response.tone} />
          <ValueRow label="Response length" value={config.response.length} />
          <ValueRow label="Web search">
        ...
        <p className="mt-3 line-clamp-2 rounded-xl bg-muted/35 p-3 text-xs leading-5 text-muted-foreground">
          {config.response.instructions}
        </p>
      </SectionCard>
      <SectionCard
        icon={ShieldAlert}
        title="Escalation & guardrails"
        description="Specify when AI should step aside and what it must avoid."
      ...
      <SectionCard
        ...
        description="Choose the fallback path when the knowledge base has no answer."
        onEdit={() => edit("notFound")}
        className="lg:col-span-2"
      >
        <div className="flex flex-col gap-4 rounded-2xl border bg-muted/20 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold">{config.notFound.action}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              {config.notFound.collectContact
                ? "Collect contact information before handoff"
                : "Do not request contact information"}
            </p>
          </div>
          <span className="flex size-9 items-center justify-center rounded-full bg-amber-500/10 text-amber-600">
  ...
  return (
    <section className="mx-auto max-w-7xl space-y-6 pb-8">
      <SectionTitle
        ...
        title="Configuration"
        details="Train, customize, and control how Atlas Support works"
        lg
      />
      <div className="flex md:flex-row flex-col md:justify-between gap-5 col-reverse">
        <TabMenu
          tabs={tabs}
          activeTab={activeTab}
      ...
      {activeTab === "general" && (
        <GeneralTab config={config} edit={setEditingSection} />
      )}
      {activeTab === "knowledge" && (
        <KnowledgeTab sources={sources} setSources={setSources} />
      )}
      {activeTab === "behavior" && (
        <BehaviorTab config={config} edit={setEditingSection} />
      )}
      {activeTab === "widget" && (
